...
def Start_Monitoring(oldWindow):
    try:
        oldWindow.destroy()
    except:
        pass
    ...
    cv2.waitKey(1)
    label_widget.after(1, Start_Monitoring(None))
    root.mainloop() 
# if(mark attendence):
    # Start_Monitoring()
def Main():
    ...
    adminLogin = Button(root, text="Admin Login",command=lambda: Start_Monitoring(root))
    adminLogin.place(x=850, y=200)
    adminLogin.pack()
    # label_widget.place(x=23 ,y=173)
    # print("Success")
